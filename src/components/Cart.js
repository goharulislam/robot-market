...
function total_price(o,i){
  switch(o){
      ...
      //parseFloat(10.25+1.25).toFixed(2);
      setTotalPrice(prevTotalPrice => (parseInt(prevTotalPrice) + (parseInt(i.price))).toFixed(2));
		break;
    case 'minus':
      setTotalItems(prevTotalItems => prevTotalItems - 1);
      setTotalPrice(prevTotalPrice => (parseInt(prevTotalPrice) - (parseInt(i.price))).toFixed(2));
...
return(
  <div className="col-md-2">
    ...
    <div className="alert alert-secondary m-0 mb-1 p-1">Total Price: ฿{totalPrice}</div>
